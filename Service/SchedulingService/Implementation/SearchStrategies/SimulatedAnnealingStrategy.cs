...
public class SimulatedAnnealingStrategy : ISearchStrategy
{
    ...
    public SimulatedAnnealingStrategy(ConstraintEvaluator evaluator, double initialTemp = 100, double coolingRate = 0.995)
    {
    ...
    public bool Iterate()
    {
        ...
        {
            double acceptanceProb = Math.Exp((_current.TotalPenalty - newPenalty) / _temperature);
            if (_random.NextDouble() < acceptanceProb)
            {
                _current = neighbor;
                _current.TotalPenalty = newPenalty;
            }
